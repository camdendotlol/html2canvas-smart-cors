...
import { parsePathForBorder, parsePathForBorderDoubleInner, parsePathForBorderDoubleOuter, parsePathForBorderStroke } from '../border';
import { calculateBorderBoxPath, calculateContentBoxPath, calculatePaddingBoxPath } from '../bound-curves';
import { contentBox } from '../box-sizing';
import { isClipEffect, isOpacityEffect, isTransformEffect } from '../effects';
import { FontMetrics } from '../font-metrics';
import { calculateObjectFitBounds } from '../object-fit';
import { reversePath, transformPath } from '../path';
...
export class CanvasRenderer extends Renderer {
    ...
    renderReplacedElement(container, curves, image) {
        if (image) {
            ...
            const isSVGContainer = container instanceof SVGElementContainer ||
                (container instanceof ImageElementContainer && container.isSVG);
            if (isContainerWSizes || isSVGContainer) {
                const box = contentBox(container);
                const path = calculatePaddingBoxPath(curves);
                this.path(path);
                const { src, dest } = calculateObjectFitBounds(container.styles.objectFit, container.intrinsicWidth, container.intrinsicHeight, box.width, box.height);
                this.ctx.save();
                this.ctx.clip();
    ...
    async renderNodeBackgroundAndBorders(paint) {
        ...
        if (hasBackground || styles.boxShadow.length) {
            ...
            styles.boxShadow
                ...
                const shadowPaintingArea = transformPath(borderBoxArea, shadow.offsetX.number - maskOffset + (shadow.inset ? 1 : -1) * shadow.spread.number, shadow.offsetY.number + (shadow.inset ? 1 : -1) * shadow.spread.number, shadow.spread.number * (shadow.inset ? -2 : 2), shadow.spread.number * (shadow.inset ? -2 : 2));
                if (shadow.inset) {
